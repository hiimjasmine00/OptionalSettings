...
$on_mod(Loaded) {
    auto mod = Mod::get();
    if (auto res = mod->registerCustomSettingType("optional-int", &OptionalIntSetting::parse); res.isErr()) {
        log::logImpl(Severity::Error, mod, "Failed to register custom setting type 'optional-int': {}", res.unwrapErr());
    }
}
